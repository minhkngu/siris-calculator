...
### Backend & Database
- **Express (Node.js)**: Server API mạnh mẽ.
- **Supabase**: Backend-as-a-Service cung cấp cơ sở dữ liệu PostgreSQL.
- **Better-SQLite3**: (Nếu có sử dụng fallback cục bộ).
- **tsx**: Chạy TypeScript trực tiếp trên Node.js.

## 📁 Cấu trúc thư mục

```text
├── server.ts              # Entry point của server, cấu hình API và Vite middleware
├── src/
│   ├── App.tsx            # Luồng chính và giao diện ứng dụng
│   ├── components/        # Các thành phần giao diện dùng chung
│   ├── lib/               # Thư viện tiện ích (utils, helpers)
...
2. **Cấu hình môi trường:**
   Tạo file `.env` từ `.env.example` và điền các thông tin cần thiết:
   - `SUPABASE_URL`: Đường dẫn API của dự án Supabase.
   - `SUPABASE_ANON_KEY`: Mã khóa công khai của Supabase.
   - `GEMINI_API_KEY`: Mã khóa API Google Gemini (nếu sử dụng tính năng AI).
...
| `SUPABASE_ANON_KEY` | API Key (Anon) để truy cập dữ liệu |
